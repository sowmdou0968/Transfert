...
/**
 * Partner form controller specs
 */
class PartnerControllerSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "PartnerController GET" should {

    "render the index page from a new instance of controller" in {
      val controller = new PartnerController(new StubComponents())
      val request = FakeRequest().withCSRFToken
      val home = controller.partnerGet().apply(request)
    ...
    "render the index page from the application" in {
      val controller = inject[PartnerController]
      val request = FakeRequest().withCSRFToken
      val home = controller.partnerGet().apply(request)
  ...
  "PartnerController POST" should {
    "process form" in {
      val request = {
        FakeRequest(POST, "/partner")
