...
/**
 * Reciever form controller specs
 */
class RecieverControllerSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "RecieverController GET" should {

    "render the index page from a new instance of controller" in {
      val controller = new RecieverController(new StubComponents())
      val request = FakeRequest().withCSRFToken
      val home = controller.recieverGet().apply(request)
    ...
    "render the index page from the application" in {
      val controller = inject[RecieverController]
      val request = FakeRequest().withCSRFToken
      val home = controller.recieverGet().apply(request)
  ...
  "RecieverController POST" should {
    "process form" in {
      val request = {
        FakeRequest(POST, "/reciever")
