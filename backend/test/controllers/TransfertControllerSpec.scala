...
/**
 * Transfert form controller specs
 */
class TransfertControllerSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "TransfertController GET" should {

    "render the index page from a new instance of controller" in {
      val controller = new TransfertController(new StubComponents())
      val request = FakeRequest().withCSRFToken
      val home = controller.transfertGet().apply(request)
    ...
    "render the index page from the application" in {
      val controller = inject[TransfertController]
      val request = FakeRequest().withCSRFToken
      val home = controller.transfertGet().apply(request)
  ...
  "TransfertController POST" should {
    "process form" in {
      val request = {
        FakeRequest(POST, "/transfert")
